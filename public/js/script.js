document.addEventListener('DOMContentLoaded', () => {
  ...
  async function addArtist1() {
    ...
    try {
      ...
      if (data?.artist) {
        ...
        showMessage(data?.error || 'Artist not found', 'error');
      }
    } catch (error) {
      showMessage('Error fetching artist - check console.', 'error');
    } finally {
  ...
  async function addArtist2() {
    ...
    // Check for duplicate
    const currentArtist1 = artist1Name.textContent || "";
    if (currentArtist1.toLowerCase() === query.toLowerCase()) {
      return showMessage('Please choose a different artist!', 'error');
    }
    ...
    try {
      const data = await fetchArtistByName(query);
      if (data?.artist) {
        // Server-side duplicate check redundancy or pre-check
        if (data.artist.name.toLowerCase() === currentArtist1.toLowerCase()) {
          showMessage('Please choose a different artist!', 'error');
          return;
        }
  ...
  if (battleBtn) {
    battleBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        showMessage('Battle request failed — check console.', 'error');
      }
    });
  }
  ...
  if (backBtn) {
    backBtn.addEventListener('click', () => {
      ...
      winnerFollowers.textContent = "";
      winnerImg.classList.remove('show');
    });
  }
});